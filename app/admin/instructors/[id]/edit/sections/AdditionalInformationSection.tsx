...
export default function AdditionalInformationSection({ instructorId, data, onSuccess }: Props) {
  ...
  useEffect(() => {
    reset({
      languagesKnown: (data?.languagesKnown || []).map((item: string) => ({ value: item, label: item })),
      proficientLanguages: (data?.proficientLanguages || []).map((item: string) => ({ value: item, label: item })),
  ...
  const onSubmit = async (form: any) => {
    try {
      ...
      await updateInstructorAdditionalInformation(instructorId, {
        ...
        proficientLanguages: form.proficientLanguages.map((item: any) => item.value),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        <label className="block mb-2 text-sm font-medium">Languages Known</label>
        <Controller
          control={control}
          name="languagesKnown"
          render={({ field }) => (
            <ReactSelect {...field} isMulti options={LANGUAGE_OPTIONS} classNamePrefix="react-select" />
          )}
        />
      </div>

      <div>
        <label className="block mb-2 text-sm font-medium">Proficient Languages</label>
        <Controller
          control={control}
      ...
      <div>
        <label className="block mb-2 text-sm font-medium">Member of Driving Association</label>
        <Controller
          control={control}
      ...
      <div>
        <label className="block mb-2 text-sm font-medium">Driving Associations</label>
        <Input {...register("drivingAssociations")} placeholder="Separate with commas" />
      </div>
